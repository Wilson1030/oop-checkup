...
**它能查出什么** —— 用 C 思维写 Java 最常见的 6 种问题：

- **贫血模型**：类只有字段没有行为，逻辑全堆在 Service 里
- **散装参数**：同样一组参数在十几个方法里传来传去
- **用 if/switch 代替多态**：类型判断越多，新增类型要改的地方越多
- **全局可变状态**：`static` 非 `final` 字段，等价于 C 的全局变量
- **封装破损**：`public` 可变字段直接暴露内部
- **main 过度承担**：200 行的入口方法，而不是用对象组织

每一条都注明公认标准出处（Fowler《重构》、贫血模型论文、Java 教科书通识），
可追溯、可查证，而不是个人观点。

免费、MIT 协议、离线可用、结果确定——**不依赖任何 LLM**。
